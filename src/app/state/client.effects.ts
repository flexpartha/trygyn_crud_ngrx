...
export class ClientEffects {
  ...
  loadClients$ = createEffect(() => {
    return this.action$.pipe(
      ofType(loadClients),
      mergeMap((action) => {
        return this.clientSrv.getClients().pipe(
          map((clients) => {
  ...
  addPost$ = createEffect(() => {
    return this.action$.pipe(
      ofType(addClient),
      mergeMap((action) => {
        return this.clientSrv.postClients(action.client).pipe(
          map((client) => {
  ...
  updateClient$ = createEffect(() => {
    return this.action$.pipe(
      ofType(updateClients),
      mergeMap((action) => {
        return this.clientSrv.updateClients(action.client).pipe(
          map((client: any) => {
  ...
  deleteClient$ = createEffect(() => {
    return this.action$.pipe(
      ...
      switchMap((action) => {
        return this.clientSrv.deleteClient(action.id).pipe(
          map((id: any) => {
